...
// ── Load PDF.js from CDN dynamically (no npm install needed) ───────────────
let _pdfjsLib = null;
...
async function renderPageToBase64(pdfDoc, pageNum, scale = 2.5) {
  ...
  return canvas.toDataURL('image/png').split(',')[1]; // base64 PNG, no prefix
}

// ── Step 1: Extract ECG signals from PDF ────────────────────────────────────
// PDF → PDF.js renders each page to a real PNG canvas image → Claude Vision analyzes.
// Primary: Flask backend (ecg_extractor.py / OpenCV).
// Fallback: PDF.js renders pages to real images → Claude Vision with image blocks.
async function extractSignalsFromPDF(pdfFile, apiBase) {
  ...
  // Step A: Use PDF.js to render every PDF page to a real PNG image (2.5× scale for clarity)
  const pdfArrayBuffer = await pdfFile.arrayBuffer();
  const pdfjsLib = await getPdfJs();
  const pdfDoc   = await pdfjsLib.getDocument({ data: new Uint8Array(pdfArrayBuffer) }).promise;
  const numPages = pdfDoc.numPages;

  // Render all pages (cap at 6 to avoid hitting token limits)
  const maxPages = Math.min(numPages, 6);
  const pageImages = []; // array of base64 PNG strings
  for (let p = 1; p <= maxPages; p++) {
    const b64 = await renderPageToBase64(pdfDoc, p, 2.5);
    pageImages.push(b64);
  }

  // Step B: Build image content blocks — one real PNG image per page
  const imageBlocks = pageImages.map(b64 => ({
    type: 'image',
    source: { type: 'base64', media_type: 'image/png', data: b64 },
  }));

  // Step C: Choose prompt based on ECG type
  // 1-page single-lead (Kardia/AliveCor) vs multi-lead standard ECG
  const isSingleLead = numPages === 1;

  const prompt = isSingleLead
    ? `You are an expert clinical cardiologist. This image shows a single-lead or rhythm-strip ECG.
Carefully trace the waveform and respond ONLY with valid JSON — no markdown, no explanation:
{
  "summary": "<1-2 sentence ECG interpretation>",
  "findings": ["<finding 1>", "<finding 2>"],
  "heartRate": "<e.g. 72 bpm>",
  "prInterval": "<e.g. 0.16s>",
  "qrsDuration": "<e.g. 0.09s>",
  "qtInterval": "<e.g. 0.40s>",
  "leads": {
    "lead0": [<100 amplitude values in mV, floats, tracing the visible waveform>],
    "lead1": [<same 100 values — duplicate for display>],
    "lead2": [<same 100 values>],
    "lead3": [<same 100 values>],
    "lead4": [<same 100 values>],
    "lead5": [<same 100 values>]
  },
  "fs": 100
}
Amplitude range is typically −2.0 to +2.0 mV. Output ONLY the JSON.`
    : `You are an expert clinical cardiologist. These images show a standard 6-lead or 12-lead ECG printout.
Carefully trace each visible lead waveform and respond ONLY with valid JSON — no markdown, no explanation:
{
  ...
  "qtInterval": "<e.g. 0.40s>",
  "leads": {
    "lead0": [<100 evenly-sampled amplitude values in mV for Lead I>],
    "lead1": [<100 values for Lead II>],
    "lead2": [<100 values for Lead III>],
    "lead3": [<100 values for aVR>],
    "lead4": [<100 values for aVL>],
    "lead5": [<100 values for aVF>]
  },
  "fs": 100
}
Amplitude range is typically −2.0 to +2.0 mV. If a lead is not visible, use 100 zeros.
Output ONLY the JSON object.`;

  // Step D: Send real PNG image blocks + prompt to Claude Vision API
  const claudeRes = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: [
          ...imageBlocks,          // real PNG images, one per page
  ...
  if (!claudeRes.ok) {
    ...
    throw new Error(`Claude Vision analysis failed (HTTP ${claudeRes.status}): ${errText.slice(0, 200)}`);
  }
