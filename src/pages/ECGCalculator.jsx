...
// ── Step 1: Extract ECG signals from PDF via Flask backend (OpenCV) ─────────
// PDF is sent to ecg_extractor.py on the Flask server — pure image-processing pipeline.
// No Claude / AI API is used at any point. The backend uses OpenCV to:
//   1. Rasterize PDF pages at 300 DPI (PyMuPDF)
//   2. Remove the pink ECG grid (HSV colour masking)
            ...
            {inputMode === 'pdf' && (
              <div className="ecg-upload-card">
                ...
                <div style={{
                  ...
                  OpenCV extracts signals → your model classifies
                </div>
              ...
              {!activeResult && !activeLoading && (
                <div className="prediction-placeholder" style={{ padding:'28px 10px' }}>
                  ...
                  <p className="prediction-placeholder-text">
                    {inputMode === 'pdf' ? 'Upload an ECG PDF — OpenCV extracts the waveform signals from the image, then your model classifies them.' : 'Upload .hea + .dat files and click Analyze to see real predictions.'}
                  </p>
                </div>
              )}
